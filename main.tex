...
\begin{abstract}
Time-series data---data that allows to track how frequency of derived alleles changed through time--is one of the richest sources for inference of directional selection.
When allele is beneficial it not just starts to increase in frequency in the population, but it also increase the frequency of alleles in nearby loci due to linkage--- the hidgehicking affect.
However, most existing looks for selection inference are single-locus methods that treat loci as independent ignoring linkage information.
They infer site-specific selection coefficient in isolation and then use various window based statistics downstream to account for hidgehicking affect.
We present a new tool \SweepLink --- a two-layer HMM that capture the selection landscape together with the linkage.
We show that \SweepLink has higher power to detect weak and moderate selection and the same power to detect high selection.
Finally, we used \SweepLink to perform genome-wide selection scan for British population from data that was previously analysed by single-locus tool \bmws.
\SweepLink was able to identify the same candidates for selection as in the previous scan (LCT, SLC45A2, DHCR4, HERC2), but was able to identify other regions that are supported by other studies as affected by selection, e.g. DPYD, FADS1/2. 
\end{abstract}
