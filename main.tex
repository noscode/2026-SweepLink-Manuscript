...
\begin{abstract}
Time-series data---allele frequency trajectories tracked across multiple sampling times---are among the richest sources of information for inferring directional selection, since they capture not only how a beneficial allele itself rises in frequency, but also how it drags the frequency of nearby loci upward via linkage---an effect known as genetic hitchhiking.
Yet most existing selection-inference tools are single-locus methods that treat loci as independent, ignoring this linkage structure: they infer site-specific selection coefficients in isolation and then rely on ad hoc, window-based statistics downstream to account for hitchhiking.
...
We present \SweepLink, a two-layer Hidden Markov Model that overcomes both limitations, jointly inferring demography and linked selection genome-wide by coupling a spatial layer, which captures correlations between selection coefficients at neighboring loci, with a temporal layer based on a Wright-Fisher diffusion model.
On simulated data, \SweepLink substantially increases sensitivity to weak and moderate selection while matching existing tools' power to detect strong selection, and its pooled, genome-wide evidence yields sharply peaked posterior probabilities that remove the need for arbitrary significance thresholds.
...
\SweepLink recovers three of the previously reported selection signals (LCT, DHCR7, HERC2), does not replicate the previously reported signal at SLC45A2, and identifies additional candidate regions---including DPYD and FADS1/FADS2---that are supported by independent studies.
\end{abstract}
